    ...
    def __init__(
        ...
        if len(self.data_list) > 0:
            ...
            try:
                ...
                first_piece = list(h5_file[first_item]["pieces"].keys())[0]
                # Set the global flag based on the first piece of the first sample
                self.has_color = "colors" in h5_file[first_item]["pieces"][first_piece]
            except Exception:
                self.has_color = False
            finally:
                h5_file.close()
        ...
        if self.has_color:
            print(f">>> [DATASET] Color detected in {data_root}. Enabling color loading.")
        else:
            print(f">>> [DATASET] No color found in {data_root}. Skipping color loading.")
